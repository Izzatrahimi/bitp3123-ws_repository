package tcpclienttexttranslator;
 ...
 * @author Izzat Rahimi
 */
public class ClientTextTranslatorFrame extends JFrame implements ActionListener{

	// Private frame components
	private JLabel title, lblAns;
	private JButton bm, arabic, korean;
	...
	public ClientTextTranslatorFrame()
	{
		...
		this.bm = new JButton("Bahasa Malaysia");
		this.arabic = new JButton("Arabic");
		this.korean = new JButton("Korean");
	...
	private JPanel centerPanel(Font font)
	{
		...
		korean.setFont(font);
		bm.setOpaque(true);
		arabic.setOpaque(true);
		korean.setOpaque(true);
		panel.add(bm);
		panel.add(arabic);
		panel.add(korean);
		return panel;
	...
	{	
		if(e.getSource()== bm)
		{
			...
			ClientTextTranslatorApplication.btnPressed();

		}
		else if(e.getSource()== arabic)
		{
			updatebtn(true);
			textinput = input.getText();
			language = "arab";
